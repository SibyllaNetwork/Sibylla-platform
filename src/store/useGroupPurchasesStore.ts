...
export type GroupPurchaseStatus = 'active' | 'closing-soon' | 'completed'
...
export interface GroupPurchase {
  ...
  /** Fase "di base" (attivo/in chiusura); `completed` è derivato dal raggiungimento
   *  della soglia minima ed è recuperato a `baseStatus` in caso di recesso. */
  baseStatus: 'active' | 'closing-soon'
}

/** Ricalcola lo stato in base ai partecipanti: completato se ha raggiunto la soglia,
 *  altrimenti torna alla fase di base (attivo/in chiusura). */
const withRecomputedStatus = (gp: GroupPurchase): GroupPurchase => ({
  ...gp,
  status: gp.currentParticipants >= gp.minQuantity ? 'completed' : gp.baseStatus,
})
...
interface GroupPurchasesState {
  ...
  /** Recede da una sottoscrizione già effettuata: rimuove l'utente e decrementa i partecipanti. */
  leavePurchase: (id: string) => void
}
...
export const useGroupPurchasesStore = create<GroupPurchasesState>()(
  persist(
    (set) => ({
      ...
      joinPurchase: (id) =>
        set((s) => {
          if (s.joinedIds.includes(id)) return s
          return {
            joinedIds: [...s.joinedIds, id],
            purchases: s.purchases.map((gp) =>
              gp.id === id
                ? withRecomputedStatus({ ...gp, currentParticipants: gp.currentParticipants + 1 })
                : gp,
            ),
          }
        }),
      leavePurchase: (id) =>
        set((s) => {
          ...
          return {
            ...
            purchases: s.purchases.map((gp) =>
              gp.id === id
                ? withRecomputedStatus({ ...gp, currentParticipants: Math.max(0, gp.currentParticipants - 1) })
                : gp,
            ),
          }
        }),
    }),
    {
      ...
      version: 2,
      // Lo schema è cambiato (baseStatus + joinedIds): scarta lo stato persistito
      // precedente e riparte dal seed, così ogni gruppo ha una fase di base valida.
      migrate: () => ({ purchases: SEED_GROUP_PURCHASES, joinedIds: [] }),
    },
  ),
)
