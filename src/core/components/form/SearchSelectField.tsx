...
export default function SearchSelectField({
  ...
  // All'apertura la riga attiva è quella già selezionata (se c'è); appena si
  // digita si riparte dalla prima, così Invio non seleziona "Nessuna" per sbaglio.
  useEffect(() => {
    if (!open) return
    const i = q ? -1 : rows.findIndex((r) => r.kind === 'opt' && r.opt.value === value)
    setActive(i >= 0 ? i : 0)
  ...
  const pick = (row: Row) => {
    ...
    if (row.kind === 'create') {
      const q0 = (query ?? '').trim()
      close()                       // svuota la ricerca: al rientro si vede il valore scelto
      onCreate?.(q0)
      return
    }
