...
/* Tieni l'animazione fluida ma compatta */
...
export function MatchZonePage() {
  ...
  const presetAutostart = searchParams.get('autostart') === '1';
  ...
  /* I miei annunci come Announcement (form di display) */
  ...
  /* Pool di annunci della community (di altri utenti) su cui cercare match */
  ...
  const startMatchFor = (listing: Announcement) => {
    const oppositeType: AnnouncementType =
      listing.type === 'vendita' ? 'acquisto' : 'vendita';

    const candidates = pool.filter(
      (p) => p.type === oppositeType && p.category === listing.category,
    );
  ...
  /* Pre-selezione + autostart da query string */
  useEffect(() => {
    if (hasAutostartedRef.current) return;
    if (!presetId) return;
    const listing = myAnnouncements.find((a) => a.id === presetId);
    if (!listing) return;
    hasAutostartedRef.current = true;
    setSelectedId(presetId);
    if (presetAutostart) {
      requestAnimationFrame(() => startMatchFor(listing));
    }
    /* Pulisci la query string per evitare re-trigger su refresh */
    setSearchParams({}, { replace: true });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [presetId, presetAutostart, myAnnouncements]);
  ...
  /* Layout colonne:
     - vendita (offro lotti): la mia card va a SINISTRA, animo DESTRA
     - acquisto (cerco lotti): la mia card va a DESTRA, animo SINISTRA */
  const animatedCard =
      ...
      {isNoMatch && selected && (
        <div className="match-zone__result match-zone__result--empty" role="status">
          ...
          <p className="match-zone__result-text">
            ...
            <strong>
              {selected.type === 'vendita' ? 'richieste (acquisto)' : 'offerte (vendita)'}
            </strong>{' '}
            disponibili nella categoria{' '}
            <em>{CATEGORY_LABELS[selected.category]}</em> per fare match con la tua{' '}
      ...
      <div className="match-card__footer">
        ...
        <div className="match-card__price">
          ...
          <span className="match-card__field-value match-card__field-value--strong">
            {format(new Date(listing.checkInDate), 'd MMM', { locale: it })} →{' '}
            {format(new Date(listing.checkOutDate), 'd MMM', { locale: it })}
          </span>
        </div>
      </div>
    </div>
