// Genera lato client il PDF del "Report Pickup" (nessun dato sul server).
// Layout richiesto dallo sketch: intestazione con titolo + intervallo temporale
// (dal giorno precedente al giorno corrente, 24h), nome del gruppo alberghiero,
// tabella con colonne Mese · Room night · Contributo ADR · Importo, logo Sibylla
// in basso. Palette: oro Sibylla + tonalità blu Platform.
import { jsPDF } from 'jspdf'

// ─── Palette (identità Sibylla + blu Platform) ───────────────────────────────
const NAVY       = '#204769' // blu Platform (header notifiche)
const NAVY_MID   = '#2C6FB3' // blu Platform intermedio (accenti)
const NAVY_TINT  = '#E8EEF4' // blu Platform chiarissimo (fasce/totali)
const ROW_TINT   = '#F3F7FB' // zebra tenue
const GOLD       = '#A2864C' // oro Sibylla
const INK        = '#1E293B'
const MUTED      = '#64748B'
const BORDER     = '#D8E2EC'
...
// ─── Dati mensili (mock deterministico, coerente con la pagina Report Pickup) ──
const MESI = ['Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio', 'Giugno', 'Luglio', 'Agosto', 'Settembre', 'Ottobre', 'Novembre', 'Dicembre']

interface Riga { mese: string; rn: number; adr: number; importo: number }

function datiMensili(anno: number): Riga[] {
  return MESI.map((m, i) => {
    const stagione = i >= 5 && i <= 8 ? 1 : 0 // picco estivo
    const rn  = 320 + Math.round(120 * Math.sin(i / 1.7) + 60 * Math.cos(i / 2.3)) + stagione * 140
    const adr = 210 + (i % 5) * 14 + stagione * 45
    return { mese: `${m} ${anno}`, rn, adr, importo: rn * adr }
  })
}

// ─── Logo Sibylla (SVG inline → PNG) ──────────────────────────────────────────
// Wordmark ufficiale (simbolo oro + "Sibylla"), rasterizzato per l'embed in PDF.
...
export interface ReportPickupPdfOpts {
  /** Nome del gruppo alberghiero mostrato in intestazione. */
  gruppo?: string
  /** Data di riferimento (il report copre le 24h dal giorno prima a questa data). */
  data?: Date
}

export async function exportReportPickupPdf(opts: ReportPickupPdfOpts = {}) {
  const gruppo = opts.gruppo ?? 'Gruppo Alberghiero Sibylla'
  const oggi = opts.data ?? new Date()
  const ieri = new Date(oggi)
  ieri.setDate(oggi.getDate() - 1)
  const intervallo = `dal ${dm(ieri)} al ${dm(oggi)}`
  ...
  // ── Intestazione: titolo (sx) + intervallo temporale (dx) ──
  let y = margin + 8
  pdf.setFont('helvetica', 'bold')
  pdf.setFontSize(24)
  pdf.setTextColor(...rgb(NAVY))
  pdf.text('Pick Up Report', margin, y + 6)

  // Pill oro con l'intervallo, allineata a destra
  pdf.setFont('helvetica', 'bold')
  pdf.setFontSize(11)
  const pillTxt = intervallo
  ...
  // ── Tabella ──
  y += 24
  // Colonne: Mese · Room night · Contributo ADR · Importo
  const colMese = margin
  const wMese = 175
  const wRn   = 100
  const wAdr  = 122
  const wImp  = contentW - wMese - wRn - wAdr
  const xRnR  = colMese + wMese + wRn          // bordo destro Room night
  const xAdrR = xRnR + wAdr                      // bordo destro Contributo ADR
  const xImpR = xAdrR + wImp                      // bordo destro Importo
  const rowH  = 27

  // Header row
  pdf.setFillColor(...rgb(NAVY))
  pdf.rect(colMese, y, contentW, rowH, 'F')
  pdf.setTextColor(255, 255, 255)
  pdf.setFont('helvetica', 'bold')
  pdf.setFontSize(11)
  const hy = y + rowH / 2 + 4
  pdf.text('Mese', colMese + 12, hy)
  pdf.text('Room night', xRnR - 12, hy, { align: 'right' })
  pdf.text('Contributo ADR', xAdrR - 12, hy, { align: 'right' })
  pdf.text('Importo', xImpR - 12, hy, { align: 'right' })
  y += rowH

  // Body rows
  const righe = datiMensili(oggi.getFullYear())
  ...
  righe.forEach((r, i) => {
    ...
    pdf.text(r.mese, colMese + 12, ty)
    pdf.setFont('helvetica', 'normal')
    pdf.setTextColor(...rgb(INK))
    pdf.text(fmtNum(r.rn), xRnR - 12, ty, { align: 'right' })
    pdf.setTextColor(...rgb(NAVY_MID))
    pdf.text(fmtEur2(r.adr), xAdrR - 12, ty, { align: 'right' })
    pdf.setTextColor(...rgb(INK))
    pdf.text(fmtEur(r.importo), xImpR - 12, ty, { align: 'right' })
    // separatore riga
    pdf.setDrawColor(...rgb(BORDER))
    pdf.setLineWidth(0.5)
    pdf.line(colMese, y + rowH, xImpR, y + rowH)
    y += rowH
  })

  // Riga Totale
  const totRn  = righe.reduce((s, r) => s + r.rn, 0)
  const totImp = righe.reduce((s, r) => s + r.importo, 0)
  const totAdr = totRn ? totImp / totRn : 0
  pdf.setFillColor(...rgb(NAVY_TINT))
  pdf.rect(colMese, y, contentW, rowH, 'F')
  pdf.setDrawColor(...rgb(GOLD))
  pdf.setLineWidth(1.5)
  pdf.line(colMese, y, xImpR, y)
  const gy = y + rowH / 2 + 4
  pdf.setFont('helvetica', 'bold')
  pdf.setTextColor(...rgb(NAVY))
  pdf.text('Totale', colMese + 12, gy)
  pdf.text(fmtNum(totRn), xRnR - 12, gy, { align: 'right' })
  pdf.text(fmtEur2(totAdr), xAdrR - 12, gy, { align: 'right' })
  pdf.text(fmtEur(totImp), xImpR - 12, gy, { align: 'right' })
  y += rowH

  // Bordo perimetrale della tabella + separatori verticali fra le colonne
  pdf.setDrawColor(...rgb(BORDER))
  pdf.setLineWidth(0.75)
  const tblTop = y - rowH * (righe.length + 2)
  pdf.rect(colMese, tblTop, contentW, rowH * (righe.length + 2))
  ;[xRnR, xAdrR].forEach((x) => pdf.line(x, tblTop, x, y))

  // ── Footer: logo Sibylla in basso ──
  const logo = await logoPng()
  const footY = ph - margin
  // riga divisoria tenue sopra il footer
  pdf.setDrawColor(...rgb(BORDER))
  pdf.setLineWidth(0.5)
  ...
  pdf.save(`Report-Pickup_${dm(ieri).replace('/', '-')}_${dm(oggi).replace('/', '-')}.pdf`)
}
