...
export default function ListiniGruppi() {
  ...
  useEffect(() => {
    let cancelled = false
    apiFetchSibylla<Data>('configura/GetListiniGruppi', { method: 'POST', body: {} })
  ...
  const save = async () => {
    setSaving(true)
    try { await apiFetchSibylla('configura/SetListiniGruppi', { method: 'POST', body: data }) } catch {}
    setSaving(false)
  }

  return (
    <div className="listini-gruppi">
      <div className="listini-gruppi__breadcrumb">
        Configuratore <i className="fa-light fa-chevron-right" /> <strong>Listini gruppi</strong>
      </div>

      <div className="listini-gruppi__filters">
        <div className="listini-gruppi__field"><label>Struttura</label>
        ...
        <div className="listini-gruppi__field"><label>Tariffa</label>
        ...
        <div className="listini-gruppi__field"><label>Stagionalità</label>
        ...
        <div className="listini-gruppi__field"><label>Listino</label>
        ...
        <div className="listini-gruppi__field"><label>Camere</label>
      ...
      <div className="listini-gruppi__header"><span>Standard Sibylla</span><span>Prezzo</span></div>
      <hr className="listini-gruppi__divider" />
      {data.Standard.map((s) => (
        <div className="listini-gruppi__row" key={s.id}>
          <span>{s.nome}</span>
          <div className="listini-gruppi__cell">
            <input type="number" step="0.01" className="sib-input listini-gruppi__input" value={s.prezzo} onChange={(e) => updatePrezzo(s.id, Number(e.target.value) || 0)} />
            <span className="listini-gruppi__unit">€</span>
          </div>
        </div>
      ))}

      <div className="listini-gruppi__actions">
