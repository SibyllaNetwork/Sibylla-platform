...
function AvatarMenu({navigate}:{navigate:(p:string)=>void}) {
  ...
  return (
    <div ref={ref} style={{position:"relative"}}>
      ...
      {open&&<div style={{position:"absolute",right:0,top:"calc(100% + 8px)",width:200,background:T.white,borderRadius:12,boxShadow:"0 8px 32px rgba(32,71,105,0.15)",border:"1px solid "+T.border,overflow:"hidden",zIndex:50}}>
        <div style={{padding:"10px 14px",borderBottom:"1px solid "+T.border}}><div style={{fontSize:12,fontWeight:600,color:T.primary}}>Luca H.</div><div style={{fontSize:11,color:T.textDisabled}}>{activeStruttura}</div></div>
        {[{icon:"user",label:"Il mio profilo",page:"modifica-profilo",danger:false},{icon:"gear",label:"Impostazioni",page:"crea-struttura",danger:false},{icon:"logout",label:"Esci",page:null,danger:true}].map((item,i)=>(
          <div key={i} onClick={()=>{setOpen(false);if(item.page)navigate(item.page);}} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 14px",cursor:"pointer",color:item.danger?T.error:T.textActive,borderTop:i===2?"1px solid "+T.border:"none"}} onMouseEnter={e=>(e.currentTarget as HTMLDivElement).style.background=item.danger?T.errorLight:"#F8FCFF"} onMouseLeave={e=>(e.currentTarget as HTMLDivElement).style.background="transparent"}>
