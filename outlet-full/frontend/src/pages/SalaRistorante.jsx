...
function ModalSposta({ tavolo, onClose, onConfirm }) {
  ...
  useEffect(()=>{
    if(!selSala) return;
    api.getTavoli(selSala.id, selTurno?.id, dateStr).then(d=>setTavoli(d.filter(t=>t.status==="disponibile"&&t.id!==tavolo.id)));
  },[selSala?.id]);
...
function ModalUnisci({ tavolo, selSala, selTurno, onClose, onConfirm }) {
  ...
  useEffect(()=>{
    ...
    api.getTavoli(selSala.id, selTurno?.id, dateStr).then(d=>
